...
namespace NCM_API.Service
{
    public class NCMService : INCMService
    {
        ...
        public Nomenclatura BuscarNCM(string ncm)
        {
            ...
            if (Regex.IsMatch(ncm, @"^\d{4}\.\d{2}\.\d{2}$"))
            {
                ...
                if (existeLista != null)
                {
                    ...
                    {
                        if (checkCodigo4 != null) DescricaoFormatada.Append($"Posição: {checkCodigo4.Descricao}");
                    }
        ...
        // private bool ChecagemNCM(string ncm)
        // {

        //     if (Regex.IsMatch(ncm, @"(?:\d){8}") || Regex.IsMatch(ncm, @"^\d{4}\.\d{2}\.\d{2}$")) return true;
